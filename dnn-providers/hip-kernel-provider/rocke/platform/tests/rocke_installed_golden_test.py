...
def _add_installed_python_paths() -> None:
    ...
    # Some harness families build library kernels, so `kernels` and `builders`
    # must resolve too. They are staged under tests/library/ in an install (the
    # destination TheRock's test-artifact globs capture) and live in the sibling

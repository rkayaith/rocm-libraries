...
Two consequences of advertising this knob are worth knowing before you rely on it. First, the ingestor advertises it **only when the provider's handle exposes `getStream()`**, since benchmarking times kernels on that stream; an engine whose handle cannot supply one simply does not offer the knob, and the frontend correspondingly does not treat it as exhaustive-capable. Second, advertising the knob makes `Graph::autotune()` in `EXHAUSTIVE` mode prime the engine: it compiles a *separate* plan carrying `global.benchmarking=1`, executes it once, then discards it and compiles the plan it actually keeps from the original knob settings. The sampled winner lives on the discarded plan, so today that priming pass costs `candidates x (warmup + iterations) + 1` device executions and changes nothing about the plan autotune goes on to time. That is a known and deliberate interim state: unlike MIOpen, whose priming warms a find-db that outlives the throwaway plan, the ingestor has no cross-plan store for a winner until the disk-backed cache lands. Callers who do not want to pay it can set `HIPDNN_FORCE_BENCHMARKING=0`, which forces benchmarking off even for autotune's priming plans.

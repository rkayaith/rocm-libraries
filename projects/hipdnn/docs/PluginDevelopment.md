...
A handle used with the ingestor **must** expose `hipStream_t getStream() const` -- benchmarking brackets each candidate with HIP events on that stream, and `GenericPlanBuilder` `static_assert`s the requirement, so a handle without it fails to compile with a message naming it rather than failing later at plan-build time. The requirement sits on the ingestor templates, not on `validateHandleType()`, so a provider that never instantiates them is unaffected.

One interim cost is worth knowing before relying on this under autotune. `Graph::autotune()` in `EXHAUSTIVE` mode primes every engine advertising the knob: it compiles a *separate* plan carrying `global.benchmarking=1`, executes it once, then discards that plan and compiles the plan it actually keeps -- and times -- from the original knob settings. Discarding is deliberate rather than wasteful: a primed plan samples every candidate on each execute, so timing it would measure the sampling sweep instead of the kernel. The design therefore assumes priming's value survives *in the provider*, not in the plan object; MIOpen's search populates a find-db that outlives the throwaway plan, which is why priming pays off there. The ingestor does not yet keep its winner anywhere that outlives the plan that measured it, so an `EXHAUSTIVE` run currently pays `candidates x (warmup + iterations) + 1` extra device executions per spec and then selects by ranked order as if it had not benchmarked. `KernelIngestorStateManager` is where a surviving winner belongs -- it already caches catalogs per `(graph, device)` and outlives every plan -- and the disk-backed winner cache work is what will fill it in. Until then, `HIPDNN_FORCE_BENCHMARKING=0` suppresses the priming cost entirely.
